...
def main(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        for j, subg in enumerate(subg_iter):
            ...
            if j == len(subg_iter) - 1:
                ...
                mlog(f"epoch:{epoch+1}/{args.n_epochs}, epoch time: {epoch_time:.4f}, Iteration {j+1}/{len(subg_iter)}:training loss {loss.item()}")
        ...
        if epoch % args.val_every == 0:
            ...
            converge_val_time.append(epoch_time)
    ...
    end_time = time.time()
    mlog(f'training using time {end_time - start_time}')
    ...
    mlog(f"BestVal Test F1-mic {test_f1_mic:.4f}, Test F1-mac {test_f1_mac:.4f} {args.n_hidden}_{args.lr}_{args.dropout}_{args.node_budget}_{args.decomp}")
    mlog(converge_val_time)
    mlog(converge_val_f1)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='MOS-GCN')
    # data source params
    ...
    # sampler params
    parser.add_argument("--sampler", type=str, default="bulk", choices=['bulk'],
